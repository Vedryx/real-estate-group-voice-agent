#!/usr/bin/env bash
# Runs an independently-named agent worker wired to DeepSeek (via
# LiveKit's inference gateway), for side-by-side comparison against
# run-openai-agent.sh / run-sarvam-agent.sh.
#
...
export AGENT_NAME="real-estate-group-deepseek"
